...
const createCell = (value: number, state: State): Cell => ({
  id: uuid(),
  value,
  state
});
...
const createGridData = (size: number): GridData => {
  ...
  for (let i = 0; i < size; i++) {
    const rows: Cell[] = [];
    for (let i = 0; i < size; i++) {
      rows.push(createCell(0, State.NORMAL));
    }
    res.push(rows);
  }
...
const cloneGrid = (data: GridData): GridData => {
  return data.map((row) => row.map((cell) => ({ ...cell, state: State.NORMAL })));
};
    ...
    for (let row = 0; row < size; row++) {
      for (let col = 0; col < size; col++) {
        if (grid[row][col].value !== 0) {
          const cell: CellPosition = { ...grid[row][col], x: col, y: row };
          res.push(cell);
        }
      }
    }
    ...
    switch (dir) {
        ...
        for (let col = 0; col < size; col++) {
          ...
          for (let row = 0; row < size; row++) {
            if (grid[row][col].value === value) {
              grid[valueIndex][col].state = State.MERGE;
      ...
      case Direction.LEFT:
        for (let row = 0; row < size; row++) {
          ...
          for (let col = 0; col < size; col++) {
            if (grid[row][col].value === value) {
              grid[row][valueIndex].state = State.MERGE;
